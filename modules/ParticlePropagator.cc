...
void ParticlePropagator::Init(){
  ...
  fInputArray = ImportArray(GetString("InputArray", "ModifyBeamSpot/stableParticles"));
  ...
  fDebugOutputCollector.addVariable3D("track") ;
...
void ParticlePropagator::Finish(){
  ...
  std::string outfile = GetString ("simpleOutputFileName", "simpleOutput_PP.root") ;
...
void ParticlePropagator::Process()
{
  ...
  while((candidate = static_cast<Candidate*>(fItInputArray->Next())))
  {
   ...
   //   fDebugOutputCollector.fillContainer3D ("track", z_t,t,z);
   ...
   //fDebugOutputCollector.fillContainer3D ("track", z_t,t,z_t*1.0E3);
    ...
    else
    {
      ...
      else
      {
       ...
       /* if(t1 < 0) t1 = 1.0E99;
        ...
        if(t1 < 4.0E-9) t1 = 1.0E99;
        if(t2 < 4.0E-9) t2 = 1.0E99;
        if(t3 < 4.0E-9) t3 = 1.0E99;
        if(t4 < 4.0E-9) t4 = 1.0E99;
        if(t5 < 4.0E-9) t5 = 1.0E99;
        if(t6 < 4.0E-9) t6 = 1.0E99; 
    ...
    if(abs(candidate->Position.Eta())<1.479){
    fDebugOutputCollector.fillContainer3D ("track", z_t*1000,t, candidate->Position.T());}
        //cout<<candidate->Position.Z()<<endl;
        //getchar();}
    //fDebugOutputCollector.fillContainer ("track", t*1.0E9);
        candidate->Momentum = candidateMomentum;
